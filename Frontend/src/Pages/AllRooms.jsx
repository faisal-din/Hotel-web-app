...
const AllRooms = () => {
  ...
  const handleFilterChange = (checked, value, type) => {
    setSelectedFilters((prevFilters) => {
      ...
      if (checked) {
        ...
        // Remove the unchecked value from the filters
  ...
  // Function to check if a room matches the room type filter
  ...
  // Function to check if a room matches the price range filter
  ...
  const filterDestination = (room) => {
    ...
    if (!destination) return true; // If no destination filter is applied, show all rooms
  ...
  const filteredRooms = useMemo(() => {
    return rooms
      ...
      .sort(sortRooms);
  }, [rooms, searchParam, selectedFilters, selectedSort]);
  ...
  return (
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 1g:px-24 xl:px-32'>
      {/* Left side */}
      <div>
        ...
        {filteredRooms.map((room) => (
          <div
            key={room._id}
            className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-32 last:bottom-0'
          >
            <img
              onClick={() => {
                {
